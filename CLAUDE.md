...
项目采用 `01.CORE`（底层元能力）和 `02.DOMAINS`（垂直领域）双层解耦架构。

### 核心原则
- **单向依赖**：DOMAIN 的代理和技能可调用 CORE 层的工具，但 CORE 层绝对不可硬编码任何特定 DOMAIN 的逻辑。
- **数据隔离**：特定领域的知识、技能、提示词（如 FILM 的分镜规则、公众号的运营策略）必须收敛在各自的 `DOMAINS` 目录下，**禁止写在根目录 CLAUDE.md 等全局文件中**。

### 切换与加载时机
- 冷启动时，必须使用 `project-guide` 进行动态路由，读取 `PROJECT.spec.md` 来决定使用哪个领域的 Agent。
- 在某领域下工作时，只允许加载该领域内部的技能以及 `01.CORE` 的基础技能。不允许跨领域调用。
