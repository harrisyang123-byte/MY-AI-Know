...
**角色一致性（跨镜头）：**
1. 开始前先询问用户：该角色是否有参考图？
   - **有参考图** → Subject 部分只写姿态和情绪，不描述外貌，提示词中写 `the person from the reference image`
   - **无参考图** → 第一个镜头用完整外貌描述建立角色，后续镜头上传该图作为参考
2. Nano Banana 支持多图参考，可同时上传角色图+空间图

**场景一致性（跨镜头）：**
1. 开始前先询问用户：该空间是否有参考图？
   - **有参考图** → Location 部分只写光线变化和新增道具，不重复描述空间，提示词中写 `same location as reference image`
   - **无参考图** → 铁律：先生成无人物的空间参考图，确认后才生成带人物的镜头
...
生成提示词后，用户可能会多轮反馈调整。每轮矫正时：

1. **明确问题**：用户指出哪里不对（光源、构图、人物、氛围等）
2. **定位原因**：是提示词描述不准确，还是模型的默认行为在干扰？
3. **修改提示词**：针对性调整正向或负向提示词
4. **输出修改版**：标注改了什么、为什么这样改

### 沉淀时机

当一个镜头经过多轮矫正后达到满意效果，**主动询问用户**：

> "这个镜头调整了 X 轮，核心问题是 [问题描述]，解决方法是 [方法]。要记录到知识库吗？"

用户确认后，按 my-best-practices.md 的方法论提炼并写入。
