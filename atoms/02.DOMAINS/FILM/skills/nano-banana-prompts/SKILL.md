...
### 合成参考图（同一空间+同一人物的后续镜头）

当用户已有该空间+该人物的合成图（比如同场景前一个镜头的生成结果），**只需 1 张参考图**，不需要拆成角色图+空间图。

**为什么更优：**
- 合成图同时锁定了空间和人物，模型不需要从两张图里"拼合"信息
- 减少参考图数量 = 减少模型的信息冲突
- 同场景连续镜头天然适合这种策略
...
| 情况 | 参考图策略 | 提示词写法 |
|------|-----------|-----------|
| 同场景第 1 个带人物镜头 | 空间图 + 角色图（2张） | `the person from reference image 2, in the location from reference image 1` |
| 同场景第 2+ 个带人物镜头 | 前一个镜头的合成图（1张） | `the same person and room as reference image 1. Camera change only: [变化]` |
| 同场景第 2+ 个镜头，但人物姿态变化大 | 前一个镜头合成图 + 角色图（2张） | `the person from reference image 2, in the same room as reference image 1` |

**关键原则：** 参考图越少越好。1 张合成图 > 2 张拆分图，前提是合成图的质量已确认。
