...
描述"刚穿过/冲出云层/雾气"等动作时，不能只描述单一方向的残留，要考虑物理真实：

- 错误：只有上方有云层边缘 → 意味着已经完全脱离云层，"刚冲出"的感觉弱
- 正确：上方厚云层边缘 + 左右两侧稀薄残云正在消散 + 下方相对清晰 → 物理真实，"刚冲出"的感觉强

原则：**从动作的物理过程出发，而不是从画面构图出发。** 穿过云层后周围应该还有残留，不是只有"穿过的方向"才有痕迹。

适用：穿过云层、穿过雾气、穿过雨幕、穿过烟尘等任何"穿过"类动作。
...
叠化是动态效果，静态图生成"叠化中段"效果不稳定，模型容易把两个画面混成一张奇怪的图。

正确做法：把叠化镜头拆成两个独立镜头分别生成，后期叠加：
1. 第一段：叠化前的画面（如人物+道具，冷灰调）
2. 第二段：叠化后的画面（如油菜花金黄光晕，暖色调，完全失焦）

第一段按正常镜头生成；第二段只描述目标画面的氛围和色彩，不需要人物。

**结论：分镜稿里有叠化的镜头，提示词阶段直接拆成两个，不要尝试在一张图里表现叠化过程。**
