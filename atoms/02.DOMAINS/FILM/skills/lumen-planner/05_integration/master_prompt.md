...
根据场景复杂度选择提示词级别。不强制全量7层：

- **L1（轻量）**：短提示词（3-10行）+ 负向提示词。适用于背对观众、纯环境、简单反应镜头、有参考图锁定人像。不拆层。
- **L2（中等）**：轻量结构化——场景定义 + 视角 + 人物动作 + 光线基调 + 风格。不逐层展元素。
- **L3（完整7层）**：仅在模型反复出错或多人物精准调度时使用：
  - 第1层：场景定义（权重最高，永远放最前）
  - 第2层：视角定义（用绝对方向，不用左右）
...
**关键**：L1/L2能解决的绝不套7层。不要为了方法论完整性升级复杂度。
...
**提示词构建**：调用 Prompt Engineering 技能，先判断复杂度级别（L1/L2/L3），再按对应策略构建提示词
