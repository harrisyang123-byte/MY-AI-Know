...
| # | 技能 | 核心方法 | 何时调用 |
|---|------|---------|---------|
| 1 | Prompt Engineering | 复杂度分级策略（L1/L2/L3）、7层提示词架构 | 构建任何提示词时 |
...
6. **批量处理** — 多输出一次调用
7. **中英对照输出** — 每轮最终提示词输出时，英文正文配中文注释版。每段英文前用【中文】标注其对应含义，便于用户审稿。设计讨论用中文，提示词用英文，注释用中文。
8. **参考图优先，文本克制** — 参考图已有的视觉信息（物体外观、材质、色彩），提示词不重复描述。只写：镜头变化、构图约束、光线差异。核心句：「[Object] exactly as shown in the reference image」。文本越少，模型越依赖参考图，一致性越高。
9. **负向提示词强制配** — 有参考图时必须配负向提示词，锁定三类：物体外观不变、空间关系不变、光线条件不变。没有负向提示词=允许模型自由发挥改变参考图基准。
