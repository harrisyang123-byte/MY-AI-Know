...
**多机位参考图链式生成**：当分镜稿指定的机位角度在已有参考图中缺失时。

```
流程：
已有参考图 A（西向拍） + 参考图 B（北向南拍）
          │
          ▼ 步骤1：空间推理
  用 A+B 的空间信息推导缺失机位的构图
  （坐标系见图纸/调度平面图，绝对方向标注）
          │
          ▼ 步骤2：构建缺失机位提示词
  以最近似角度的参考图为视觉基准，
  ...
  执行生成，产出新机位参考图 C
          │
          ▼ 步骤4：链式参考
  用户确认 C 后，C 作为最终提示词的主要参考
```

**关键规则**：
- 缺失机位提示词中，空间布局必须基于坐标系推导，不能凭感觉
- 人物位置用绝对方向标注（"张十三在北侧桌后，门在南墙"）
- 生成后必须与已有参考图做一致性检查（空间关系是否自洽、人物外貌是否连贯）
